...
<script type='text/javascript'>
	...
	function searchList(pageNo) {
		var comAjax = new ComAjax();
		comAjax.setUrl("<c:url value='/admin/api/solar/company/search/paging' />");
	...
	function searchListCB(data) {
		...
		if (total == 0) {
			...
			$.each(data.result, function(key, value) {
				...
				str += "	<td><a href='/admin/rdt/solar/company/detail?company_id=" + value.company_id + "'>" + value.name + "</a></td>";
	...
	function downloadExcelFile() {
		if (confirm('기업 정보를 다운로드합니다. 기업 정보가 많으면 다소 시간이 걸릴 수도 있습니다. 다운로드하시겠습니까?')) {
			...
			location.href = "/admin/api/solar/company/excelDownload" + queryString;
		}
	}
