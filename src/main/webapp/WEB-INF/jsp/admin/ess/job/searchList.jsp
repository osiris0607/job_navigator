...
<script type='text/javascript'>
	...
	function downloadExcelFile() {
		if (confirm('일자리 정보를 다운로드합니다. 일자리 수가 많으면 다소 시간이 걸릴 수도 있습니다. 다운로드하시겠습니까?')) {
			...
			location.href = "/admin/api/ess/job/excelDownload" + queryString;
		}
	}
	...
	function searchList(pageNo) {
		...
		comAjax.setUrl("<c:url value='/admin/api/ess/job/search/paging' />");
	...
	function searchListCB(data) {
		...
		if (total == 0) {
			...
			$.each(data.result, function(key, value) {
				...
				str += "	<td><a href='/admin/rdt/ess/job/detail?job_id=" + value.job_id + "'>" + value.name + "</a></td>";
