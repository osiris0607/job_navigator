...
<script type='text/javascript'>
	...
	function getDetail() {
		...
		comAjax.setUrl("<c:url value='/user/api/ess/notification/trend/search/detail'/>");
	...
	function fnSetPreNext() {
		var comAjax = new ComAjax();
		comAjax.setUrl("<c:url value='/user/api/ess/trend/preNext' />");
	...
	function fnSetPreNextCallback(data) {
		...
		if (data.result.pre != null) {
			$("#pre").attr("href", "/user/rdt/ess/notification/trend/detailMobile?trend_id=" + data.result.pre.trend_id);
			$("#pre").html(data.result.pre.title);
		}
		
		if (data.result.next != null) {
			$("#next").attr("href", "/user/rdt/ess/notification/trend/detailMobile?trend_id=" + data.result.next.trend_id);
...
<body>
  <div id="wrap">
    <header class="sub">
      <div class="headerSubWrap">
        <button class="prev" onclick="location.href='location.href='/user/rdt/ess/mobile/main/mainMobile';"></button>
        <h1>태양광산업 동향</h1>
      </div>
    </header>
  ...
  <section>
      <div class="referenceWrap">
        ...
        <div class="btnWrap">
          <a href="/user/rdt/ess/notification/trendMobile" class="btn viewList">목록보기</a>
        </div>
   </section>
</div>
