...
<script type='text/javascript'>
		...
		$.each(companyList, function(key, value) {
			if (value.company_id == id){
				...
				if ( gfn_isNull(value.associated_job_1) == false ) {
					...
					str += "		<a href='/user/rdt/ess/jobDictionary/detail?job_id=" + value.associated_job_1 + "' class='btn_detail' target='_blank'>상세보기 <i class='fa fa-angle-right' aria-hidden='true'></i></a>";
					str += "	</div>";
				}
				...
				if ( gfn_isNull(value.associated_job_2) == false ) {
					...
					str += "		<a href='/user/rdt/ess/jobDictionary/detail?job_id=" + value.associated_job_2 + "' class='btn_detail' target='_blank'>상세보기 <i class='fa fa-angle-right' aria-hidden='true'></i></a>";
					str += "	</div>";
				}
				...
				if ( gfn_isNull(value.associated_job_3) == false ) {
					...
					str += "		<a href='/user/rdt/ess/jobDictionary/detail?job_id=" + value.associated_job_3 + "' class='btn_detail' target='_blank'>상세보기 <i class='fa fa-angle-right' aria-hidden='true'></i></a>";
					str += "	</div>";
				}
	...
	function searchMap() {
		...
		comAjax.setUrl("<c:url value='/user/api/ess/company/search/region' />");
	...
	function searchList(pageNo) {
		var comAjax = new ComAjax();
		comAjax.setUrl("<c:url value='/user/api/ess/company/search/paging' />");
	...
	function categoryChange(categoryId) {
		if (categoryId != ""){
			...
			<c:forEach items="${commonCode}" var="code">
				<c:if test="${fn:escapeXml(code.master_id) == 'M000003' and fn:escapeXml(code.menu_id) == 'M000003'}">
			...
			<c:forEach items="${commonCode}" var="code">
				<c:if test="${code.master_id == 'M000002' and fn:escapeXml(code.menu_id) == 'M000003'}">
					str += '<option value="${code.detail_id}"><c:out value="${code.name}"></c:out></option>';
				</c:if>
			</c:forEach>	
			category.append(str);
          ...
          <div id="tab01" class="js-tabcontent">
            <div class="bsMap">
              ...
              <div id="Gyeongbuk" class="mapCard">
               ...
               <!--  <p class="cardTitle"><em class="en">D.</em>태양광 발전사업 &gt; <em class="en">D.1.</em> 사업개발/컨설팅</p> -->
                <div class="provinceWrap">
